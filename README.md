...
**Create models using machine learning methods to predict track popularity.**
...
Spotify has made incredible API resources available.
...
5. Track popularity and numerical physical features have virtually no correlation; however, track, album, and artist popularity are highly correlated, which is not surprising. There is also a small correlation between track popularity and track number, which is also not surprising because the majority of popular songs are typically the first ones on an album.

6. We developed a machine learning algorithm that was able to accurately forecast track popularity. We selected XGBClassifier to train our data because ensemble methods are very effective at analysing multi-feature data with non-linear relationships, and because XGBoost has recently dominated the data science field with extreme superiority. We were able to achieve very excellent accuracy scores for both cross-validated and test data. The popularity of albums is the best predictor.


 7.We vectorized text features into numeric to reinforce our models, and we are using API data, which may more accurately reflect the most current trend. 

8. Regretfully, users cannot obtain location data using the Spotify API; otherwise, it would be beneficial to examine variations in music preferences between states and the world.

Overall, our analysis of Spotify API data has shown some fascinating tendencies for the current music industry and has also produced a top-notch model for predicting track popularity. With any luck, this will shed some light on the state of the music business both now and in the future.
